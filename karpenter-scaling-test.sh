  ...
  METRICS_FILE="karpenter_scaling_metrics_$(date +%Y%m%d_%H%M%S).csv"
  ...
  get_instance_type() {
      ...
      if [ -z "$instance_type" ]; then
          # Tenta extrair de labels específicos do Karpenter
          instance_type=$(kubectl get node $node_name -o jsonpath='{.metadata.labels.karpenter\.sh/capacity-type}')
          
          # Se ainda não tem instance_type, tenta obter mais informações do Karpenter
          if [ -z "$instance_type" ]; then
  ...
  # Função para obter informações específicas do Karpenter
  get_karpenter_info() {
      echo "Obtendo informações do Karpenter..."
      
      # Verificar NodePools (substitui os grupos de nós do Cluster Autoscaler)
      echo "NodePools do Karpenter:"
      kubectl get nodepools -o wide
      
      # Obter estatísticas de provisioners
      echo "Detalhes dos NodePools:"
      kubectl describe nodepools
      
      # Verificar classes de nós EC2
      echo "EC2NodeClasses do Karpenter:"
      kubectl get ec2nodeclasses
      
      # Coletar estatísticas de provisioning atuais
      NODES_PROVISIONED=$(kubectl get nodes -l karpenter.sh/provisioner-name -o name | wc -l)
      log_metric "karpenter_nodes_provisioned" $NODES_PROVISIONED
      
      # Coletar capacidade total provisionada
      TOTAL_CPU_PROVISIONED=$(kubectl get nodes -l karpenter.sh/provisioner-name -o jsonpath='{.items[*].status.capacity.cpu}' | tr ' ' '\n' | awk '{s+=$1} END {print s}')
      log_metric "karpenter_total_cpu_provisioned" $TOTAL_CPU_PROVISIONED
      
      # Verificar eventos relacionados ao Karpenter
      echo "Eventos recentes do Karpenter:"
  ...
  # Verificar informações do Karpenter
  get_karpenter_info
  ...
  kubectl scale deployment nginx-test -n test-scaling --replicas=0
  log_metric "target_replicas" 1

  echo "Aguardando 15 minutos para que o Karpenter remova nodes..."
